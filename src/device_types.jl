
# Sensors
typealias LargeMotor Device{TachoMotor, Driver{:lego_ev3_l_motor}}
typealias MediumMotor Device{TachoMotor, Driver{:lego_ev3_m_motor}}

# Motors
typealias UltrasoundSensor Device{LegoSensor, Driver{:lego_ev3_us}}
typealias ColorSensor Device{LegoSensor, Driver{:lego_ev3_color}}
typealias TouchSensor Device{LegoSensor, Driver{:lego_ev3_touch}}
typealias GyroSensor Device{LegoSensor, Driver{:lego_ev3_gyro}}
